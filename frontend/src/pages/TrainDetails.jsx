...
const TrainDetails = () => {
    ...
    const handleBook = async () => {
        ...
        if (formValues) {
            try {
                const res = await bookingService.book(id, formValues.seats, formValues.travelDate, formValues.passengerName, formValues.passengerContact);
    ...
    if (!train) return <div className="text-center py-16 text-slate-500">Train not found.</div>;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
                    ...
                    <div className="lg:col-span-2 p-5 border-r border-slate-100">
                        ...
                        <div className="relative pl-6">
                            <div className="absolute left-[5px] top-3 bottom-3 w-0.5 bg-slate-200 rounded-full"></div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="bg-white p-3 ml-4 rounded-lg border border-slate-100 shadow-sm flex items-center justify-between">
                                        <div>
                                            <p className="font-bold text-railway-dark text-sm uppercase">{train.source}</p>
                                            <p className="text-[10px] font-semibold text-slate-400">Source</p>
                                        </div>
                                ...
                                {sortedStations.map((station, index) => (
                                    <div key={index} className="relative">
                                        ...
                                        <div className="bg-white p-3 ml-4 rounded-lg border border-slate-100 shadow-sm flex items-center justify-between hover:border-railway-primary/20 transition-colors">
                                            <p className="font-semibold text-slate-700 text-sm uppercase">{station.stationName}</p>
                                            <div className="flex space-x-4 text-right">
                                                <div>
                                                    <p className="font-semibold text-slate-600 text-xs">{station.arrivalTime}</p>
                                                    <p className="text-[9px] uppercase font-bold text-slate-300">Arr</p>
                                                </div>
                                                <div>
                                                    <p className="font-semibold text-slate-600 text-xs">{station.departureTime}</p>
                                                    <p className="text-[9px] uppercase font-bold text-slate-300">Dep</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="bg-white p-3 ml-4 rounded-lg border border-slate-100 shadow-sm flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] font-semibold text-slate-400">Destination</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-bold text-green-600 text-sm">{train.arrivalTime}</p>
                                            <p className="text-[10px] font-semibold text-slate-400">Arrival</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {/* Right: Booking Panel */}
                    <div className="bg-slate-50 p-5 flex flex-col">
                        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4">
                            ...
                            <p className="text-[10px] text-slate-400 mt-1.5 font-medium text-center">of {train.totalSeats} total seats</p>
                        </div>
                        ...
                        <div className="mt-auto">
                            <button
                                ...
                                className={`w-full py-3.5 rounded-xl font-black text-sm transition-all shadow-lg active:scale-95 ${train.availableSeats > 0
